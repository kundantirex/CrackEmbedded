...
<head>
    ...
    <style>
        ...
        * {{
        ...
        .banner {{
            background: linear-gradient(120deg, #0B1120 0%, #13314A 55%, #1B4D63 100%);
...
for path in base_dir.rglob("*.html"):
    if path.stat().st_size == 0:
        ...
        with open(path, "w") as f:
            f.write(content)
        count += 1
        print(f"Populated {rel_path} with {title}")

print(f"Successfully populated {count} empty HTML files.")
